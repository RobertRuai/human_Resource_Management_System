    ...
    <form action="{{ route('leaves.update', $leave->id) }}" method="POST">
            ...
            <select name="employee_id_number" id="employee_id_number" class="form-select" required>
                ...
                @foreach($employees as $employee)
                    <option value="{{ $employee->id }}" 
                        {{ (old('employee_id_number', $leave->employee_id_number) == $employee->id) ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            ...
            <select name="type_of_leave" id="type_of_leave" class="form-select" required>
                ...
                <option value="Annual" {{ (old('type_of_leave', $leave->type_of_leave) == 'Annual') ? 'selected' : '' }}>Annual</option>
                <option value="Sick" {{ (old('type_of_leave', $leave->type_of_leave) == 'Sick') ? 'selected' : '' }}>Sick</option>
                <option value="Maternity" {{ (old('type_of_leave', $leave->type_of_leave) == 'Maternity') ? 'selected' : '' }}>Maternity</option>
                <option value="Paternity" {{ (old('type_of_leave', $leave->type_of_leave) == 'Paternity') ? 'selected' : '' }}>Paternity</option>
                <option value="Unpaid" {{ (old('type_of_leave', $leave->type_of_leave) == 'Unpaid') ? 'selected' : '' }}>Unpaid</option>
                <option value="Other" {{ (old('type_of_leave', $leave->type_of_leave) == 'Other') ? 'selected' : '' }}>Other</option>
            </select>
        </div>
